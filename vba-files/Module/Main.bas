...
Do Until (MessageComplete Or (count > 40))
  ...
  dwError = WinHttpWebSocketReceive(hWebSocketHandle, rgbBuffer(0), dwBufferLength, VarPtr(dwBytesTransferred), VarPtr(eBufferType))
  If (dwError <> ERROR_SUCCESS) Then
    dwError = GetLastError
    GoTo quit
  End If
...
dwError = WinHttpWebSocketQueryCloseStatus(hWebSocketHandle, usStatus, _
             rgbCloseReasonBuffer(0), dwCloseReasonBufferLength, VarPtr(dwCloseReasonLength))
If (dwError <> ERROR_SUCCESS) Then
  GoTo quit
End If
' report result
